...
module "lambda" {
  ...
  lambda_role_lab           = "arn:aws:iam::${local.id_conta}:role/LabRole"
...
module "sqs" {
  source                      = "./modules/sqs"
  lambda_trigger_arn          = module.lambda.lambda_trigger_arn
  lambda_insert_dynamo_arn    = module.lambda.lambda_insert_dynamo_arn
}
...
module "dynamodb" {
  ...
  partition_key  = { name = "pedido", type = "S" }
  sort_key       = { name = "datetime", type = "S" }
